...
if [ "$1" != "force" ] && [ $(date +%s) -lt "$timelimit" ]; then echo "[$(basename "$0")] $(date) - DONE: ISO up-to-date. READY!"; exit 0; fi
...
echo "[$(basename "$0")] $(date) - DONE: purged leftovers"
...
echo "[$(basename "$0")] $(date) - DONE: started alpine-runner with alpine/edge image"
...
echo "[$(basename "$0")] $(date) - DONE: push alpine_install_tools.sh in alpine-runner"
sudo lxc exec alpine-runner -- ./install_tools.sh; wait $!
echo "[$(basename "$0")] $(date) - DONE: tools installed in alpine-runner"
...
echo "[$(basename "$0")] $(date) - DONE: pull + purge logs from alpine-runner"
...
echo "[$(basename "$0")] $(date) - DONE: snapshot + publishing as ISO"
...
sudo lxc image delete iso-alpine-edge
echo "[$(basename "$0")] $(date) - DONE: purged alpine-runner and iso-alpine-edge"
...
if [ $? -eq 0 ]
then
    echo "[$(basename "$0")] $(date) - DONE: iso-alpine-stage successfully created"
    sudo lxc image delete iso-alpine-utils
    echo "[$(basename "$0")] $(date) - DONE: purged stale iso-alpine-utils"
    sudo lxc image alias rename iso-alpine-stage iso-alpine-utils
    echo "[$(basename "$0")] $(date) - DONE: renamed in iso-alpine-utils - READY!"
else
    echo "[$(basename "$0")] $(date) - FAIL: NO iso-alpine-stage AVAILABLE"
fi
