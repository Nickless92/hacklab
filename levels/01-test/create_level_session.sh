...
tmux new-session -s hacklab -n level01 -d 'lxc exec lvl01-c02 -- tshark -i eth0 > ausgabe.txt; bash -i'\; split-window -v -d 'lxc shell lvl01-c01'\; split-window -h -d 'lxc exec lvl01-c03 -- tshark -i eth0' \; select-pane -D
